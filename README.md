...
导出弹窗用一行汇总各颜色累计消耗；舞者切换顺序每人一行，以时间戳和左右双色圆点展示，长序列可横向滚动。悬停、触摸或键盘聚焦圆点可查看左右手颜色、状态和新增根数。每只手首次取棒或换到另一种非黑色计 1 根，换回之前的颜色也重新计数；连续同色和纯走位关键帧不重复计数，黑色表示未持棒、不计根数。退场后再次入场按重新取棒统计。统计覆盖全部已保存关键帧，超出歌曲时长的关键帧会提示检查。
