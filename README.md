...
2. 在下方时间轴空白处点击新建段落；选中段落后用“当前作为入点”（`[`）和“当前作为出点”（`]`）打点；拖动左右边缘也可调整。
...
- 拖动段落块整体移动时间区间，左右边缘调整入点/出点；拖动会限制在歌曲范围和前后片段边界；打点仍校验时间和重叠。编排列表顺序保持不变。
...
- **XLSX**：仅包含已编排段落，首张表使用原工具的六列表头、歌曲/BPM 标题及合并结构；最后一张表附光棒累计用量和逐人切换时间。不包含音频或完整走位数据，重新导入 XLSX 只恢复段落编排。
...
空格切换播放／暂停，长按不重复切换；在输入框、文本域、选择框、可编辑内容、弹窗或输入法组合输入期间不触发。播放按钮悬停提示显示该快捷键。

## 一次性光棒用量

导出弹窗按关键帧统计各颜色累计消耗，并可展开每位舞者的时间顺序、状态、左右手颜色和当次新增根数。每只手首次取棒或换到另一种非黑色计 1 根，换回之前的颜色也重新计数；连续同色和纯走位关键帧不重复计数，黑色表示未持棒、不计根数。退场后再次入场按重新取棒统计。统计覆盖全部已保存关键帧，超出歌曲时长的关键帧会提示检查。

Excel 最后一张“光棒用量统计”工作表包含颜色汇总和完整逐人切换明细；首张六列编排表保持与原 Python 工具兼容。统计是由关键帧即时计算的，不保存成额外项目字段。
