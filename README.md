...
2. 在下方时间轴空白处点击新建段落；选中段落后可输入入点、出点秒数，或用当前播放位置打点；拖动左右边缘也可调整。
3. 上方当前段落卡片中点击类型、拍数、双语歌词、技 / 动作编排或备注进行编辑。
4. 导入 TXT 或打开 LRC 选择歌词；LRC 同一时间戳只能选一条日文，其余自动组成中文。
5. 导入本地歌曲，使用时间轴定位、拖动和缩放进行对时；选中段落后可点击卡片的“循环”练习。A/B 练习直接在播放位置点击“选 A”“选 B”，再开始或退出循环。
...
- 拖动段落块整体移动时间区间，左右边缘调整入点/出点；无效或重叠区间会拒绝并回退。编排列表顺序保持不变。
